...
export function splitYahavConcatenatedRow(line: string): string[] | null {
  ...
  if (parseAmountNumber(amounts[0]) === 0) {
    // Shape like: "0.00 <credit>" (or "0.00 <credit> <balance>" with balance omitted here).
    return [date, ref, desc, '0.00', amounts[1]];
  }
  // Two-number rows in Yahav are commonly either:
  // - debit + running balance (credit cell omitted), or
  // - credit + running balance (debit cell omitted), especially salary/credit rows.
  const incomeLike = /משכורת|זיכוי|הפקדה|החזר/i.test(desc);
  if (incomeLike) {
    return [date, ref, desc, '0.00', amounts[0], amounts[1]];
  }
  return [date, ref, desc, amounts[0], '0.00', amounts[1]];
}
