// One-off verification: run the Isracard scraper with the production options
// (incl. defaultTimeout/navigationRetryCount from v2.0.72) against the active
// DB config. Usage (inside backend container):
//   npx ts-node prisma/verify-isracard-config-scrape.ts
...
async function main() {
  ...
  try {
    const cfg = await prisma.scraperConfig.findFirst({
      where: { companyId: 'isracard', isActive: true, syncEnabled: true },
    ...
    if (!cfg) {
      throw new Error('No active Isracard scraper config found');
    }
    ...
    console.log('verify-isracard-config-scrape');
    console.log(
      JSON.stringify(
        {
          configId: cfg.id,
