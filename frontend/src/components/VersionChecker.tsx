...
const MANUAL_UPDATE_ONE_LINER =
  'cd /opt/finance-app && git fetch origin main && git checkout -B main origin/main && git reset --hard origin/main && (docker compose exec -T backend npx prisma migrate deploy || true) && docker compose build --no-cache backend frontend nginx && docker compose up -d';
