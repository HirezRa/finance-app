...
### Dashboard — פילטרי סקירה מגיבים לנתונים

- תוקן מצב שבו כפתורי `יום/שבוע/מחזור/שנה/הכל` היו לחיצים אך לא שינו נתונים בפועל.
- עבור טווחים שאינם `מחזור`, הדשבורד טוען עסקאות לפי טווח תאריכים (`/transactions` עם pagination) ומחשב מחדש:
  - הכנסות/הוצאות/יתרה זמינה
  - פירוק קטגוריות
  - גרף שבועי
  - רשימת עסקאות אחרונות
- שיפור UI מסונכרן:
  - מצב הבחירה נשמר ב־URL (`?range=`).
  - ניווט מחזור (חיצים/מחזור נוכחי) נשאר רק ב־`מחזור`.
  - בכרטיס "נשאר להוציא" מוצגת אינדיקציית טווח בפילטרים שאינם מחזור.

## [2.0.78] - 2026-06-10

### Settings UX — סדר לשוניות הגיוני

- לשוניות מסך ההגדרות סודרו מחדש לזרימה שימושית יותר:
  **פרופיל → תצוגה → תקציב → התראות → אבטחה → AI → n8n → עדכוני תוכנה → לוגים → נתונים**.
- הלשונית הפעילה וההתנהגות הפנימית לא השתנו; זהו שיפור סדר וניווט בלבד.

## [2.0.77] - 2026-06-10

### Dashboard UX — כפתורי סקירה לחיצים ומסונכרנים

- **סקירה / טווח זמן:** כפתורי `יום/שבוע/מחזור/שנה/הכל` הוסבו למצב אינטראקטיבי (ללא `disabled`), עם `aria-selected` תקין וסנכרון מצב דרך query param (`?range=`) כך שהבחירה נשמרת ולא "קופצת".
- **מניעת חוסר סנכרון בתצוגה:** ניווט חודשי (חיצים/מחזור נוכחי) מוצג רק במצב `מחזור`; במצבים אחרים מוצג טווח נבחר בלבד, כדי למנוע סתירה בין כפתור נבחר לבין כותרת מחזור.
- **הסרת קיצורי דרך מיותרים בראש המסך:** בוטל תפריט הקיצורים העליון (הכפילויות באנגלית), ונשאר ניווט אחיד בצד ימין בעברית.

## [2.0.76] - 2026-06-10

### אבטחה — תיקון פריסה להקשחת Bull Board

- **Bull Board:** בוטלה לחלוטין חשיפת פורט host (ללא `ports` בשירות), ולכן הממשק אינו נגיש יותר מהרשת החיצונית כברירת מחדל.
- **תיקון תאימות:** החלפה זו מתקנת בעיית Compose (`invalid IP address: localhost`) שנוצרה בגישת bind קודמת.
- **Nginx hardening נשאר פעיל** מ־2.0.75 (`server_tokens off`, כותרות הרשאות/COOP/CORP, חסימת dotfiles).
...
- **הבעיה (לוג 13:28–13:30 UTC):** סנכרון ויזה כאל נכשל 3/3 ניסיונות בשלב `fetch_transactions` — ה‑API של כאל החזיר גוף תשובה ריק/קטוע (WAF/האטה) ופרסור ה‑JSON קרס עם הודעה עירומה. ההתחברות עצמה הצליחה (~14 שניות).
- **התיקון (בפורק, `hirez-v1.0.27`):**
  - קריאות ה‑API של כאל (מסגרות, עסקאות ממתינות, עסקאות חודשיות) מנוסות עד 3 פעמים עם השהיה (2/4 שניות); תשובה "רכה" (`statusCode !== 1`) בעסקאות חודשיות נחשבת גם היא ברת‑ניסיון‑חוזר.
  - כשל בעסקאות ממתינות לא מפיל את הסריקה — ממשיכים בלי נתוני ממתינות.
  - שגיאות פרסור של `fetchPost` כוללות כעת URL, סטטוס HTTP, אורך גוף ותחילתו — אבחון מיידי בלוגים במקום `Unexpected end of JSON input`.
- **תלות:** `israeli-bank-scrapers` / `@hirez10/israeli-bank-scrapers` → **`hirez-v1.0.27`** (PR ‏#50 בפורק).
- **חדש:** `backend/prisma/verify-visacal-config-scrape.ts` — סקריפט אימות חי לויזה כאל (כמו של יהב/ישראכרט).
