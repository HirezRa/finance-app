...
$guest = "cd $projectOnGuest && git fetch origin main && git checkout -B main origin/main && git reset --hard origin/main && docker compose build --no-cache backend && docker compose down --remove-orphans && docker compose up -d && sleep 8 && curl -s --max-time 10 --connect-timeout 5 http://localhost/api/v1/health"
